...
//The function returns true if the base_addr is a valid general purpose timer
//*****************************************************************************
bool gp_timer_config_32(uint32_t base_addr, uint32_t mode, bool count_up, bool enable_interrupts, uint32_t countDown)
{
  ...
  // ADD CODE
	...
	gp_timer->TAILR = countDown;	// set top value and countDown

	if(enable_interrupts)
		gp_timer->IMR |= TIMER_IMR_TATOIM;
  //*********************
    gp_timer->CTL |= TIMER_CTL_TAEN;	// enable

    
  return true;  
}
bool gp_timer_config_16(uint32_t base_addr, uint32_t mode, bool count_up, bool enable_interrupts, uint32_t countDown){
	uint32_t timer_rcgc_mask;
  uint32_t timer_pr_mask;
  TIMER0_Type *gp_timer;
...
uint32_t timer_value(uint32_t base_addr, bool timer_A){
	TIMER0_Type *gp_timer;
  ...
  gp_timer = (TIMER0_Type *)base_addr;
	...
	// Depending on timer, return A or B
	if (timer_A)
		return gp_timer->TAV;
	else
		return gp_timer->TBV;
