...
static __INLINE uint8_t accel_reg_read(uint8_t reg)
{
	...
	tx_data[0] = reg | 0x80;
	tx_data[1] = 0;
	spiTx(ACCEL_SPI_BASE, tx_data, size, data);
  // ADD CODE
	accel_CSN_high();
  return data[1]; // Modify to return the register value
}
...
static __INLINE void accel_reg_write(uint8_t reg, uint8_t data)
{
	...
	spiTx(ACCEL_SPI_BASE, tx_data, size, &n);
	accel_CSN_high();
  // ADD CODE
}
...
int16_t accel_read_x(void)
{
	...
	uint8_t x_H;
	uint16_t x;
	x_L = accel_reg_read(0x28);
	x_H = accel_reg_read(0x29);
	
	x = (x_H << 8) + x_L;
  // ADD CODE
  return x; // Modify to return the x value
...
int16_t accel_read_y(void)
{
	...
	uint8_t y_H;
	uint16_t y;
	y_L = accel_reg_read(0x2A);
	y_H = accel_reg_read(0x2B);
	
	y = (y_H << 8) + y_L;
  // ADD CODE
  return y; // Modify to return the Y value
...
int16_t accel_read_z(void)
{
  uint8_t z_L;
	uint8_t z_H;
	uint16_t z;
	z_L = accel_reg_read(0x2C);
	z_H = accel_reg_read(0x2D);
	
	z = (z_H << 8) + z_L;
  // ADD CODE
  return z; // Modify to return the Z value
